...
router.post('/register', exports.verifyToken, async (req, res) => {
    try {
        ...
        // Create user
        const result = await pool.query(`INSERT INTO users (id, restaurant_id, email, password_hash, name, role, phone, permissions, active)
 ...
 */
router.get('/users', exports.verifyToken, async (req, res) => {
    try {
        if (req.user?.role !== 'admin' && req.user?.role !== 'manager') {
            return res.status(403).json({ error: 'Unauthorized' });
        }
 ...
 * Get user details
 */
router.get('/users/:id', exports.verifyToken, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const result = await pool.query('SELECT id, email, name, role, phone, active, last_login, created_at FROM users WHERE id = $1', [id]);
        if (result.rows.length === 0) {
 ...
 */
router.put('/users/:id', exports.verifyToken, async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.put('/users/:id/password', exports.verifyToken, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (isSelf && !isAdmin) {
            ...
            const passwordMatch = await bcrypt_1.default.compare(currentPassword, userResult.rows[0].password_hash);
            if (!passwordMatch) {
