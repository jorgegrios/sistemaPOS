  ...
  try {
    ...
    await pool.query(
      `INSERT INTO menus (id, restaurant_id, name, description)
       VALUES ($1, $2, $3, $4)`,
      [menuId, restaurantId, 'Main Menu', 'Our featured menu items']
    );
    ...
    for (let i = 0; i < categories.length; i++) {
      ...
      await pool.query(
        `INSERT INTO menu_categories (id, menu_id, name, display_order)
         VALUES ($1, $2, $3, $4)`,
        [categoryId, menuId, categories[i], i]
      );
    }
    ...
    for (const item of menuItems) {
      await pool.query(
        `INSERT INTO menu_items (id, category_id, name, description, price, available, display_order)
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      await pool.query(
        `INSERT INTO tables (id, restaurant_id, table_number, capacity, status)
         VALUES ($1, $2, $3, $4, $5)`,
        [tableId, restaurantId, `Table ${i}`, 4, 'available']
      );
    }
    ...
    for (let i = 0; i < userRoles.length; i++) {
      const userId = uuidv4();
      userIds.push(userId);
      const passwordHash = await bcrypt.hash(`password_${userRoles[i]}`, 10);
    ...
    for (let i = 0; i < 3; i++) {
      await pool.query(
        `INSERT INTO payment_terminals (id, terminal_id, provider, device_type, location_id, status)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [
          uuidv4(),
