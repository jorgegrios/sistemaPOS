/**
 * Bar Served Orders Page
 * Display bar orders that have been served (all items prepared and served)
...
export const BarServedOrdersPage: React.FC = () => {
  ...
  const loadServedOrders = async () => {
    try {
      ...
      console.error('[BarServedOrdersPage] Error loading served bar orders:', err);
      console.error('[BarServedOrdersPage] Error details:', err.message, err.stack);
      if (!loading) {
        toast.error(`Error al cargar órdenes entregadas: ${err.message || 'Error desconocido'}`);
      }
      setServedOrders([]);
    } finally {
      setLoading(false);
              ...
              >
                ...
                <div className="flex-1 space-y-2 overflow-y-auto max-h-[400px]">
                  {order.items.map((item) => (
                    <div
                      key={item.id}
                      className="rounded-lg p-3 border-2 bg-blue-50 border-blue-400 text-blue-800"
                    >
                      <div className="mb-2">
                        ...
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span className="px-2 py-1 rounded font-bold text-xs bg-blue-500 text-white">
                            Entregado
                          </span>
                        </div>
                        {item.notes && (
                      ...
                      <div className="w-full py-2 rounded-lg bg-blue-500 text-white text-center font-bold text-sm">
                        ✓ Entregado
                      </div>
                    </div>
                  ))}
